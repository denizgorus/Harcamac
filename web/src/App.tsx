...
import { AreaChart, Area, BarChart, Bar, CartesianGrid, Cell, Pie, PieChart, ReferenceLine, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts'
...
import { Asset, AssetTransaction, Cadence, Category, defaultCategories, Entry, FlowKind } from './types'
...
const REMEMBERED_ACCOUNTS_KEY = 'harcamac-remembered-accounts'
const PENDING_ACCOUNT_KEY = 'harcamac-pending-account-email'
const REMEMBER_LOGIN_KEY = 'harcamac-remember-login'
...
function rememberedAccounts() {
  try{return (JSON.parse(localStorage.getItem(REMEMBERED_ACCOUNTS_KEY)||'[]') as string[]).filter(Boolean)}catch{return []}
}

function rememberAccount(email:string) {
  const normalized=email.trim().toLocaleLowerCase('tr-TR')
  if(!normalized)return
  localStorage.setItem(REMEMBERED_ACCOUNTS_KEY,JSON.stringify([normalized,...rememberedAccounts().filter(item=>item!==normalized)].slice(0,6)))
}
...
function tradingChartDomain(values:ChartValue[],topMargin=0.1,bottomMargin=0.16) {
  const finite=values.filter((value):value is number=>typeof value==='number'&&Number.isFinite(value)&&value>0)
  if(!finite.length)return [0.99,1.01] as [number,number]
  const min=Math.min(...finite),max=Math.max(...finite)
  if(min===max){
    const pad=Math.max(Math.abs(min)*0.015,0.01)
    return [Math.max(min-pad,min*0.985,Number.EPSILON),max+pad] as [number,number]
  }
  const range=max-min
  const visible=Math.max(0.2,1-topMargin-bottomMargin)
  const lowerPad=range*bottomMargin/visible
  const upperPad=range*topMargin/visible
  return [Math.max(min-lowerPad,min*0.5,Number.EPSILON),max+upperPad] as [number,number]
}

function signedChartDomain(values:ChartValue[],topMargin=0.12,bottomMargin=0.14) {
  const finite=values.filter((value):value is number=>typeof value==='number'&&Number.isFinite(value))
  if(!finite.length)return [-1,1] as [number,number]
  const min=Math.min(...finite),max=Math.max(...finite)
  if(min===max){
    const pad=Math.max(Math.abs(min)*0.18,1)
    return [min-pad,max+pad] as [number,number]
  }
  const range=max-min
  const visible=Math.max(0.2,1-topMargin-bottomMargin)
  return [min-range*bottomMargin/visible,max+range*topMargin/visible] as [number,number]
}

function compactChartTick(value:number) {
  ...
  if(abs>=10_000)return `${Math.round(value/1000).toLocaleString('tr-TR')}Bin`
  if(abs>=1000)return value.toLocaleString('tr-TR',{maximumFractionDigits:1})
  if(abs>=100)return value.toLocaleString('tr-TR',{maximumFractionDigits:0})
  if(abs>=10)return value.toLocaleString('tr-TR',{maximumFractionDigits:1})
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => { if(data.session?.user.email)rememberAccount(data.session.user.email);setSession(data.session); setAuthReady(true) })
    const { data } = supabase.auth.onAuthStateChange((_event, value) => { if(value?.user.email&&localStorage.getItem(REMEMBER_LOGIN_KEY)!=='false')rememberAccount(value.user.email);setSession(value) })
...
function Auth({theme,setTheme}:{theme:'light'|'dark';setTheme:(theme:'light'|'dark')=>void}) {
  ...
  const [email, setEmail] = useState(()=>{const pending=localStorage.getItem(PENDING_ACCOUNT_KEY)||'';if(pending)localStorage.removeItem(PENDING_ACCOUNT_KEY);return pending||rememberedAccounts()[0]||''})
  ...
  const [remember,setRemember]=useState(()=>localStorage.getItem(REMEMBER_LOGIN_KEY)!=='false')
  ...
  async function submit(e: FormEvent) {
    ...
    if(!result.error){localStorage.setItem(REMEMBER_LOGIN_KEY,String(remember));if(remember)rememberAccount(email)}
  ...
  return <main className={`auth-shell ${mode === 'login' ? 'auth-login' : 'auth-signup'}`}>
    ...
    <section className="auth-panel"><form className="auth-form" onSubmit={submit}>
      ...
      {mode === 'login' && <label className="remember-login"><input type="checkbox" checked={remember} onChange={event=>setRemember(event.target.checked)}/><span>Beni hatırla</span></label>}
...
function FinanceApp({ session, theme, setTheme }: { session: Session; theme:'light'|'dark'; setTheme:(theme:'light'|'dark')=>void }) {
  ...
  const [confirmRequest,setConfirmRequest]=useState<{title:string;message:string;confirmLabel:string;onConfirm:()=>void|Promise<void>}|null>(null)
  ...
  async function deleteEntry(id:string) {
    ...
    if(error)showAppError('Hareket silme',error,{hareket_id:id})
    else loadData()
  }

  async function deleteAsset(asset:Asset) {
    const ids=asset.source_ids?.length?asset.source_ids:[asset.id]
    const {error}=await supabase.from('assets').delete().in('id',ids)
    if(error)showAppError('Varlık silme',error,{sembol:asset.symbol,varlık:asset.name,varlık_id:asset.id})
    else loadData()
  }
  ...
  const topAction = page === 'settings' ? null : page === 'assets'
  ...
  return <div className="app-shell sidebar-auto">
    <aside className="sidebar"><div className="logo"><span className="brand-mark turkish-lira-symbol">₺</span><span>Harcamaç</span></div><nav data-tour="navigation">{nav.map(item => <button key={item.id} className={page === item.id ? 'active' : ''} onClick={() => setPage(item.id)}><item.icon /> <span>{item.label}</span></button>)}</nav><div className="sidebar-user"><div className="avatar">{(session.user.user_metadata.full_name || session.user.email || 'H')[0].toUpperCase()}</div><div><b>{session.user.user_metadata.full_name || 'Hesabım'}</b><small>{session.user.email}</small></div><button title="Hesap değiştir" onClick={() => setAccountSwitchOpen(true)}><ArrowDownUp /></button></div></aside>
    <main className="workspace"><header><div><span className="mobile-logo">Harcamaç</span><h1>{title}</h1><p>{subtitle}</p></div><div className="header-actions">{(page === 'transactions' || page === 'dashboard') && <button className="category-button" data-tour="category-action" title="Kategoriler" onClick={() => setCategoryOpen(true)}><Tags /><span>Kategoriler</span></button>}{topAction&&<button className="primary" data-tour="top-action" onClick={topAction.onClick}><Plus /> <span>{topAction.label}</span></button>}</div></header>
      {loading ? <div className="loading">Verileriniz getiriliyor…</div> : <>
        {page === 'dashboard' && <Dashboard entries={entries} assets={assets} />}
        {page === 'transactions' && <Transactions entries={entries} categories={categories} onEdit={e => { setEditEntry(e); setEntryOpen(true) }} onDelete={id => setConfirmRequest({title:'Hareket silinsin mi?',message:'Bu kayıt kalıcı olarak silinecek. Bütçe ve grafik toplamları hemen güncellenir.',confirmLabel:'Sil',onConfirm:()=>deleteEntry(id)})} />}
        {page === 'recurring' && <Recurring entries={entries} onEdit={e => { setEditEntry(e); setEntryOpen(true) }} onDelete={id => setConfirmRequest({title:'Düzenli kayıt silinsin mi?',message:'Bu düzenli gelir veya harcama kalıcı olarak silinecek. İstersen daha sonra yeniden ekleyebilirsin.',confirmLabel:'Sil',onConfirm:()=>deleteEntry(id)})} />}
        {page === 'assets' && <Assets userId={userId} assets={assets} entries={entries} onBuy={asset=>{setAssetPreset(asset);setAssetOpen(true)}} onDelete={asset => setConfirmRequest({title:'Varlık silinsin mi?',message:`${asset.symbol} pozisyonu ve bağlı lot kayıtları portföyden kaldırılacak.`,confirmLabel:'Sil',onConfirm:()=>deleteAsset(asset)})} onChanged={loadData} />}
        {page === 'settings' && <SettingsPage theme={theme} setTheme={setTheme} email={session.user.email || ''} catAlerts={catAlerts} setCatAlerts={setCatAlerts} openCategories={() => setCategoryOpen(true)} openGuide={() => setGuideOpen(true)} openAccountSwitcher={()=>setAccountSwitchOpen(true)} />}
      </>}
    </main>
    ...
    {confirmRequest&&<ConfirmModal {...confirmRequest} close={()=>setConfirmRequest(null)}/>}
    {accountSwitchOpen&&<AccountSwitchModal currentEmail={session.user.email||''} close={()=>setAccountSwitchOpen(false)}/>}
  </div>
}
...
function Dashboard({ entries, assets }: { entries: Entry[]; assets: Asset[] }) {
  ...
  const current = currentMonthlyEntries(entries,selectedDate)
  ...
  const monthLabel=selectedDate.toLocaleDateString('tr-TR',{month:'long',year:'numeric'})
  const maxMonth=monthKey(new Date()), canGoNext=selectedMonth<maxMonth
  function shiftMonth(step:number){const next=new Date(selectedDate);next.setMonth(next.getMonth()+step);const key=monthKey(next);setSelectedMonth(key>maxMonth?maxMonth:key)}
  return <div className="page-content">
    <section className="dashboard-period"><b>{monthLabel}</b><div><button className="icon-button subtle" title="Önceki ay" onClick={()=>shiftMonth(-1)}><ChevronLeft/></button><input type="month" aria-label="Özet dönemi" max={maxMonth} value={selectedMonth} onChange={event=>{const value=event.target.value||maxMonth;setSelectedMonth(value>maxMonth?maxMonth:value)}}/><button className="icon-button subtle" title="Sonraki ay" disabled={!canGoNext} onClick={()=>shiftMonth(1)}><ChevronRight/></button></div></section>
    <section className="metric-grid"><Metric label="Gelir" value={income} tone="green"/><Metric label="Gider" value={expense} tone="red"/><Metric label="Aylık denge" value={income-expense} tone="ink"/><Metric label="Toplam varlık" value={assetTotal} tone="blue"/></section>
    <section className="chart-grid">{defaultWidgets.map(widget=><article className="panel chart-card" data-tour={widget.id==='flow'?'charts':undefined} key={widget.id}><ChartHeader widget={widget}/><ChartBody widget={widget} entries={entries} anchor={selectedDate}/></article>)}
    </section>
...
function filterRange(entries:Entry[],range:ChartRange,anchor=new Date()){if(range==='month')return currentMonthlyEntries(entries,anchor);if(range==='week'){const start=new Date(anchor);start.setDate(anchor.getDate()-6);return effectiveEntriesInRange(entries,start,anchor)}const start=new Date(anchor.getFullYear(),anchor.getMonth()-5,1,12);return effectiveEntriesInRange(entries,start,monthEnd(anchor.getFullYear(),anchor.getMonth()))}
function chartData(widget:ChartWidget,entries:Entry[],anchor=new Date()){const list=filterRange(entries,widget.range,anchor);if(widget.metric==='categoryExpense')return Object.values(list.filter(e=>e.kind==='expense').reduce((a,e)=>{const key=e.category_name||'Diğer';a[key]||={name:key,value:0,color:e.category_color||'#7b837e'};a[key].value+=Number(e.amount);return a},{} as Record<string,{name:string;value:number;color:string}>)).sort((a,b)=>b.value-a.value);if(widget.range==='all'){return Array.from({length:6},(_,i)=>{const d=new Date(anchor.getFullYear(),anchor.getMonth()-5+i,1);const month=list.filter(e=>{const x=localDate(e.entry_date);return x.getMonth()===d.getMonth()&&x.getFullYear()===d.getFullYear()});return {name:d.toLocaleDateString('tr-TR',{month:'short'}),Gelir:month.filter(e=>e.kind==='income').reduce((s,e)=>s+Number(e.amount),0),Gider:month.filter(e=>e.kind==='expense').reduce((s,e)=>s+Number(e.amount),0)}})}return Object.values(list.reduce((a,e)=>{const key=localDate(e.entry_date).toLocaleDateString('tr-TR',{day:'numeric',month:'short'});a[key]||={name:key,Gelir:0,Gider:0};a[key][e.kind==='income'?'Gelir':'Gider']+=Number(e.amount);return a},{} as Record<string,{name:string;Gelir:number;Gider:number}>))}
function ChartBody({widget,entries,anchor}:{widget:ChartWidget;entries:Entry[];anchor:Date}) {
  const data=chartData(widget,entries,anchor), colors=['#dc5b4f','#e7a23d','#418a73','#477ea8','#8b69a8','#6b7c55']
...
function EntryRows({entries,actions}:{entries:Entry[];actions?:{onEdit:(e:Entry)=>void;onDelete:(id:string)=>void}}){return <div className="entry-list">{entries.map(e=><div className="entry-swipe" key={e.id}><div className="entry-row"><div className={`entry-icon ${e.kind}`}><CircleDollarSign/></div><div className="entry-main"><b>{e.title}</b><span>{e.category_name} · {new Date(`${e.entry_date}T12:00:00`).toLocaleDateString('tr-TR',{day:'numeric',month:'short'})} · {e.cadence==='recurring'?'Düzenli':'Tek seferlik'}</span></div><div className="entry-side"><div className={`entry-amount ${e.kind}`}><b>{e.kind==='expense'?'-':'+'}{trMoney.format(Number(e.amount))}</b></div></div></div>{actions&&<div className="swipe-actions"><button title="Düzenle" onClick={()=>actions.onEdit(e)}><Pencil/><span>Düzenle</span></button><button className="danger" title="Sil" onClick={()=>actions.onDelete(e.id)}><Trash2/><span>Sil</span></button></div>}</div>)}</div>}

function Recurring({entries,onEdit,onDelete}:{entries:Entry[];onEdit:(e:Entry)=>void;onDelete:(id:string)=>void}) { const list=entries.filter(e=>e.cadence==='recurring'); const inc=list.filter(e=>e.kind==='income').reduce((s,e)=>s+Number(e.amount),0), exp=list.filter(e=>e.kind==='expense').reduce((s,e)=>s+Number(e.amount),0); return <div className="page-content"><section className="metric-grid two"><Metric label="Düzenli gelir" value={inc} tone="green"/><Metric label="Düzenli gider" value={exp} tone="red"/></section><section className="panel"><PanelTitle title="Düzenli hareketler" subtitle="Aylık planınız"/><EntryRows entries={list} actions={{onEdit,onDelete}}/></section></div> }

function AccountSwitchModal({currentEmail,close}:{currentEmail:string;close:()=>void}) {
  const accounts=rememberedAccounts()
  async function switchTo(email?:string){if(email)localStorage.setItem(PENDING_ACCOUNT_KEY,email);await supabase.auth.signOut();close()}
  return <Modal title="Hesap değiştir" close={close} className="account-switch-modal"><div className="account-switch-content"><p>Kayıtlı hesaplardan birini seç veya farklı bir hesapla giriş yap.</p><div className="account-list">{accounts.map(email=><button key={email} disabled={email===currentEmail.toLocaleLowerCase('tr-TR')} onClick={()=>switchTo(email)}><span className="avatar">{email[0]?.toUpperCase()}</span><span><b>{email===currentEmail.toLocaleLowerCase('tr-TR')?'Şu anki hesap':email}</b><small>{email}</small></span></button>)}</div><div className="form-actions"><button type="button" onClick={close}>Vazgeç</button><button type="button" className="primary" onClick={()=>switchTo()}>Başka hesapla giriş yap</button></div></div></Modal>
}

const assetRanges: MarketRange[] = ['1G','1H','1A','3A','1Y','Maks.']
...
function Assets({userId,assets,entries,onBuy,onDelete,onChanged}:{userId:string;assets:Asset[];entries:Entry[];onBuy:(asset:Asset)=>void;onDelete:(asset:Asset)=>void;onChanged:()=>void}) {
  ...
  const [performanceUnit,setPerformanceUnit]=useState<'amount'|'percent'>('amount')
  ...
  const [performanceData,setPerformanceData]=useState<{label:string;flow:number|null;performance:number|null;performanceRate:number|null;flowRate?:number;changeRate?:number}[]>([])
  ...
  const latestPerformancePoint = [...performanceData].reverse().find(point => point.flow !== null || point.performance !== null)
  const rangeGain = latestPerformancePoint?.performance ?? gain
  const rangeGainRate = latestPerformancePoint?.performanceRate ?? gainRate
  const chartDataKey = analysisMode==='flow'?'flow':'chartValue'
  const activeChartData = performanceData.map(point=>({...point,chartValue:analysisMode==='performance'?(performanceUnit==='percent'?point.performanceRate:point.performance):point.flow,changeRate:analysisMode==='flow'?point.flowRate:point.performanceRate})).filter((point): point is typeof point & {chartValue:number}=>typeof point.chartValue==='number'&&Number.isFinite(point.chartValue))
  const chartValues = activeChartData.map(point=>point.chartValue)
  const chartPositive = (analysisMode==='flow'?Number(latestPerformancePoint?.flow||0):rangeGain) >= 0
  function changeRange(next:MarketRange){setPerformanceLoading(true);setRange(next)}
  ...
  useEffect(()=>{let cancelled=false;setPerformanceLoading(true);const end=Date.now(),start=portfolioRangeStart(range,positions).getTime();Promise.all(positions.map(async asset=>({asset,history:await fetchAssetHistory(asset,range)}))).then(series=>{if(cancelled)return;const purchaseTimes=positions.flatMap(asset=>{if(!asset.purchase_date)return [];const timestamp=new Date(`${asset.purchase_date}T12:00:00`).getTime();return Number.isFinite(timestamp)&&timestamp>=start&&timestamp<=end?[timestamp]:[]});const timestamps=[...new Set([start,...purchaseTimes,...series.flatMap(item=>item.history.map(point=>point.timestamp*1000)).filter(timestamp=>timestamp>=start&&timestamp<=end),end])].sort((a,b)=>a-b);const raw=timestamps.map(timestamp=>{let flow=0,invested=0;for(const {asset,history} of series){const purchase=asset.purchase_date?new Date(`${asset.purchase_date}T12:00:00`).getTime():start;if(timestamp<purchase)continue;const point=[...history].reverse().find(item=>item.timestamp*1000<=timestamp);const assetCost=Number(asset.units)*Number(asset.average_cost);const price=(point?.price??Number(asset.average_cost))||Number(asset.current_price);invested+=assetCost;flow+=price*Number(asset.units)}const gainValue=flow-invested;return {label:historyLabel(timestamp/1000,range),flow:flow>0?flow:null,gain:flow>0?gainValue:null,invested}}).filter(point=>Number(point.flow)>0);let data=raw;if(data.length===1){const only=data[0];data=zeroRangePoints(range).map(point=>({label:point.label,flow:only.flow,gain:only.gain,invested:only.invested}))}const first=data.find(point=>Number(point.flow)>0);const baseFlow=first?.flow||0,baseGain=range==='Maks.'?0:(first?.gain||0);setPerformanceData(data.map(point=>{const performance=point.gain===null?null:point.gain-baseGain;const performanceRate=performance!==null&&point.invested?performance/point.invested*100:null;return {label:point.label,flow:point.flow,performance,performanceRate,flowRate:point.flow&&baseFlow?pointRate(point.flow,baseFlow):0,changeRate:performanceRate||0}}))}).finally(()=>{if(!cancelled)setPerformanceLoading(false)});return()=>{cancelled=true}},[portfolioHistoryKey,range])
  ...
  return <div className="page-content assets-page midas-assets">
    <section className="assets-portfolio">
      ...
      <div className={`portfolio-change ${rangeGain >= 0 ? 'positive' : 'negative'}`}><span>{performanceLoading?'Hesaplanıyor…':hidden?'******':`${rangeGain>=0?'+':''}${formatMoney(rangeGain)}`}</span><b>{performanceLoading?'…':`${rangeGainRate.toLocaleString('tr-TR',{maximumFractionDigits:2})}%`}</b></div>
      <RangeTabs value={range} onChange={changeRange} className="portfolio-range-tabs"/>
    ...
    {view==='performance'&&<section className="portfolio-analysis panel"><header><div><small>{analysisMode==='performance'?'Kar / zarar':'Toplam varlık akışı'}</small><strong>{performanceLoading?'Hesaplanıyor…':analysisMode==='performance'&&performanceUnit==='percent'?`${rangeGainRate>=0?'+':''}${rangeGainRate.toLocaleString('tr-TR',{maximumFractionDigits:2})}%`:formatMoney(analysisMode==='performance'?rangeGain:total)}</strong><span className={chartPositive?'positive':'negative'}>{analysisMode==='performance'?`${rangeGain>=0?'+':''}${formatMoney(rangeGain)}`:`${(latestPerformancePoint?.flowRate||0)>=0?'+':''}${(latestPerformancePoint?.flowRate||0).toLocaleString('tr-TR',{maximumFractionDigits:2})}%`}</span></div><div className="analysis-controls"><div className="analysis-mode"><button className={analysisMode==='performance'?'active':''} onClick={()=>setAnalysisMode('performance')}>Performans</button><button className={analysisMode==='flow'?'active':''} onClick={()=>setAnalysisMode('flow')}>Varlık akışı</button></div>{analysisMode==='performance'&&<div className="analysis-mode performance-unit"><button className={performanceUnit==='amount'?'active':''} onClick={()=>setPerformanceUnit('amount')}>TL</button><button className={performanceUnit==='percent'?'active':''} onClick={()=>setPerformanceUnit('percent')}>%</button></div>}</div></header><div className="portfolio-chart">{performanceLoading?<div className="chart-loading">Grafik hazırlanıyor…</div>:activeChartData.length?<ResponsiveContainer><AreaChart data={activeChartData} margin={{left:8,right:10}}><defs><linearGradient id="portfolioFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor={chartPositive?'#3d8b68':'#dc5b4f'} stopOpacity=".28"/><stop offset="1" stopColor={chartPositive?'#3d8b68':'#dc5b4f'} stopOpacity="0"/></linearGradient></defs><CartesianGrid vertical={false} stroke="var(--line)" strokeOpacity={0.65}/><XAxis dataKey="label" axisLine={false} tickLine={false} interval="preserveStartEnd" minTickGap={55} tickMargin={8} height={30} tick={{fill:'var(--muted)',fontSize:10}}/><YAxis orientation="right" width={54} axisLine={false} tickLine={false} tickMargin={6} tickCount={4} domain={analysisMode==='flow'?tradingChartDomain(chartValues):signedChartDomain(chartValues)} tickFormatter={analysisMode==='performance'&&performanceUnit==='percent'?value=>`${compactChartTick(value)}%`:compactChartTick} tick={{fill:'var(--muted)',fontSize:10}}/>{analysisMode==='performance'&&<ReferenceLine y={0} stroke="var(--muted)" strokeDasharray="3 3" strokeOpacity={0.55}/>}<Tooltip content={<ChartValueTooltip formatValue={analysisMode==='performance'&&performanceUnit==='percent'?value=>`${value.toLocaleString('tr-TR',{maximumFractionDigits:2})}%`:formatMoney}/>}/><Area type="monotone" dataKey={chartDataKey} stroke={chartPositive?'#3d8b68':'#dc5b4f'} strokeWidth={2.4} fill="url(#portfolioFill)" dot={false} activeDot={{r:4}} isAnimationActive={false}/></AreaChart></ResponsiveContainer>:<div className="chart-loading">Grafik için yeterli veri yok.</div>}</div><RangeTabs value={range} onChange={changeRange}/><div className="analysis-stats"><span>Maliyet<b>{formatMoney(cost)}</b></span><span>Güncel değer<b>{formatMoney(total)}</b></span><span>{range} kar/zarar<b className={rangeGain>=0?'positive':'negative'}>{formatMoney(rangeGain)}</b></span></div><div className="asset-allocation"><h3>Varlık dağılımı</h3><div className="donut-wrap asset-allocation-wrap"><div className="donut-stage"><div className="donut"><ResponsiveContainer><PieChart><Pie data={allocationData.length?allocationData:[{name:'Veri yok',value:1,color:'#e4e6e1'}]} innerRadius="55%" outerRadius="90%" dataKey="value" stroke="none" isAnimationActive={false}>{(allocationData.length?allocationData:[{color:'#e4e6e1'}]).map((group,index)=><Cell key={`${group.color}-${index}`} fill={group.color}/>)}</Pie></PieChart></ResponsiveContainer><div><b>{formatMoney(allocationTotal)}</b><small>toplam</small></div></div></div><div className="allocation-legend">{allocationData.length?allocationData.map(group=><div key={group.name}><i style={{background:group.color}}/><b>%{allocationTotal?Math.round(group.value/allocationTotal*100):0}</b><span>{group.name}</span><small>{formatMoney(group.value)}</small></div>):<p>Henüz dağılım verisi yok.</p>}</div></div></div><div className="cash-returns"><h3>Nakit getirileri</h3><div><span>TL nema<b>{formatMoney(cashReturns.interest)}</b></span><span>TL temettü<b>{formatMoney(cashReturns.dividend)}</b></span><span>USD nema<b>{currency==='USD'?formatMoney(cashReturns.interest):'$0,00'}</b></span><span>USD temettü<b>$0,00</b></span></div></div></section>}
    {view==='watchlist'&&<section className="watchlist-panel panel"><header><div><h2>Takip Ettiklerim</h2><p>{watchItems.length} varlık</p></div><RangeTabs value={range} onChange={setRange}/></header><label className="search watch-search"><Search/><input placeholder="Hisse, fon, kripto veya endeks ara" value={watchQuery} onChange={event=>setWatchQuery(event.target.value)}/></label>{watchResults.length>0&&<div className="watch-search-results">{watchResults.map(item=>{const watched=watchKeys.includes(`${item.kind}:${item.symbol}`);return <button key={`${item.kind}-${item.symbol}`} title={`${item.symbol} - ${item.name}`} aria-label={`${item.symbol} - ${item.name}`} onClick={()=>toggleWatch(item)}><span><b>{item.symbol}</b><small>{item.kind} · {item.name}</small></span>{watched?<BookmarkCheck/>:<Bookmark/>}</button>})}</div>}<div className="watch-list">{watchItems.map(item=>{const key=`${item.kind}:${item.symbol}`,price=watchPrices[key];return <button key={key} title={`${item.symbol} - ${item.name}`} aria-label={`${item.symbol} - ${item.name}`} onClick={()=>openWatch(item)}><span className="asset-symbol"><b>{item.symbol.slice(0,3)}</b></span><span><b>{item.symbol}</b><small>{item.name}</small></span><span><b>{price?formatMoney(price):'—'}</b><small className="positive">{item.kind}</small></span><BookmarkCheck onClick={event=>{event.stopPropagation();toggleWatch(item)}}/></button>})}{!watchItems.length&&<Empty text="Arama yaparak takip listene varlık ekleyebilirsin."/>}</div></section>}
...
function RangeTabs({value,onChange,className=''}:{value:MarketRange;onChange:(range:MarketRange)=>void;className?:string}) { return <div className={`asset-range-tabs ${className}`.trim()}>{assetRanges.map(range=><button key={range} className={value===range?'active':''} onClick={()=>onChange(range)}>{range}</button>)}</div> }
...
function MarketDiscover({formatMoney,watched,toggleWatch,openItem}:{formatMoney:(value:number)=>string;watched:string[];toggleWatch:(item:AssetCatalogItem)=>void;openItem:(item:AssetCatalogItem)=>void}) {
  ...
  return <section className="discover-panel panel"><header><div><small>Piyasalar</small><h2>Keşfet</h2></div><Compass/></header><label className="search discover-search"><Search/><input placeholder="Hisse, fon, kripto veya endeks ara" value={query} onChange={event=>setQuery(event.target.value)}/>{query&&<button title="Aramayı temizle" onClick={()=>setQuery('')}><X/></button>}</label><div className="discover-categories">{(['Tümü','Türkiye','ABD','Fonlar','Kripto'] as const).map(item=><button key={item} className={category===item?'active':''} onClick={()=>setCategory(item)}>{item}</button>)}</div>
    {!query&&category==='Tümü'&&<><section className="market-overview"><h3>Piyasa özeti</h3><div>{featured.map(item=>{const quote=quotes[`${item.kind}:${item.symbol}`];return <button key={`${item.kind}-${item.symbol}`} title={`${item.symbol} - ${item.name}`} aria-label={`${item.symbol} - ${item.name}`} onClick={()=>select(item)}><span><b>{item.symbol}</b><small>{item.name}</small></span><span><b>{quote?.price?formatMoney(quote.price):'—'}</b><small className={(quote?.change||0)>=0?'positive':'negative'}>{quote?`${quote.change>=0?'+':''}${quote.change.toLocaleString('tr-TR',{maximumFractionDigits:2})}%`:'—'}</small></span></button>})}</div></section>{recent.length>0&&<section className="recent-assets"><h3>Son aramalar</h3><div>{recent.map(item=><button key={`${item.kind}-${item.symbol}`} title={`${item.symbol} - ${item.name}`} aria-label={`${item.symbol} - ${item.name}`} onClick={()=>select(item)}><span className="asset-symbol"><b>{item.symbol.slice(0,3)}</b></span><span><b>{item.symbol}</b><small>{item.name}</small></span><ChevronRight/></button>)}</div></section>}</>}
    {(query||category!=='Tümü')&&<section className="discover-results"><h3>{query?'Arama sonuçları':category}</h3>{results.map(item=>{const key=`${item.kind}:${item.symbol}`;return <div className="discover-result-row" key={key} title={`${item.symbol} - ${item.name}`} aria-label={`${item.symbol} - ${item.name}`} onClick={()=>select(item)}><span className="asset-symbol"><b>{item.symbol.slice(0,3)}</b></span><span><b>{item.symbol}</b><small>{item.name}</small></span><small>{item.kind}</small><button className="icon-button subtle" title={watched.includes(key)?'Takipten çıkar':'Takip et'} onClick={event=>{event.stopPropagation();toggleWatch(item)}}>{watched.includes(key)?<BookmarkCheck/>:<Bookmark/>}</button></div>})}{!results.length&&<Empty text="Aramana uygun varlık bulunamadı."/>}</section>}
  </section>
}
...
function AssetRow({asset,total,formatMoney,onOpen,onEdit,onDelete}:{asset:Asset;total:number;formatMoney:(value:number)=>string;onOpen:()=>void;onEdit:()=>void;onDelete:(asset:Asset)=>void}) {
  ...
  return <div className="asset-swipe">
    ...
    <div className="asset-swipe-actions"><button title="Varlığı düzenle" onClick={event=>{event.stopPropagation();onEdit()}}><Pencil/><span>Düzenle</span></button><button className="danger" title="Varlığı sil" onClick={event=>{event.stopPropagation();onDelete(asset)}}><Trash2/><span>Sil</span></button></div>
  </div>
}
...
function AssetDetail({asset,formatMoney,watched,close,onEdit,toggleWatch,trade}:{asset:Asset;formatMoney:(value:number)=>string;watched:boolean;close:()=>void;onEdit?:()=>void;toggleWatch:()=>void;trade?:(side:'buy'|'sell')=>void}) {
  const [range,setRange]=useState<MarketRange>('1A'),[points,setPoints]=useState<{timestamp:number;label:string;price:number;value:number;changeRate?:number}[]>([]),[loading,setLoading]=useState(true),[fund,setFund]=useState<FundSnapshot|null>(null),[transactions,setTransactions]=useState<AssetTransaction[]>([])
  ...
  useEffect(()=>{let cancelled=false;setLoading(true);fetchAssetHistory(asset,range).then(history=>{if(cancelled)return;const filtered=history.filter(point=>Number.isFinite(point.price)&&point.price>0);const baseline=filtered[0]?.price||0;setPoints(filtered.map(point=>({timestamp:point.timestamp,label:historyLabel(point.timestamp,range),price:point.price,value:point.price*Number(asset.units),changeRate:pointRate(point.price,baseline)})))}).finally(()=>{if(!cancelled)setLoading(false)});return()=>{cancelled=true}},[asset.id,range])
  ...
  useEffect(()=>{let cancelled=false;const ids=asset.source_ids?.length?asset.source_ids:[asset.id];if(asset.id.startsWith('watch-'))return;supabase.from('asset_transactions').select('*').in('asset_id',ids).order('transaction_date',{ascending:false}).then(({data})=>{if(!cancelled)setTransactions((data||[]) as AssetTransaction[])});return()=>{cancelled=true}},[asset.id,asset.source_ids?.join('|')])
  const fallbackPrice=Number(asset.current_price)||Number(asset.average_cost)
  const first=points.find(point=>point.price>0)?.price||fallbackPrice,last=points[points.length-1]?.price||fallbackPrice,periodChange=last-first,positive=periodChange>=0
  const periodRate=first?periodChange/first*100:0
  const chartPoints=points.length>1?points:fallbackPrice>0?zeroRangePoints(range).map(point=>({...point,price:fallbackPrice,value:fallbackPrice*Number(asset.units),changeRate:0})):[]
  const initialInfo={date:asset.purchase_date||'',units:Number(asset.units),price:Number(asset.average_cost),total:Number(asset.units)*Number(asset.average_cost)}
  ...
  return <div className="asset-detail-backdrop"><section className="asset-detail"><header><button className="icon-button subtle" title="Geri" onClick={close}><ArrowLeft/></button><div><b>{asset.symbol}</b><small>{asset.name}</small></div><div className="detail-actions">{onEdit&&<button className="icon-button subtle" title="Düzenle" onClick={onEdit}><Pencil/></button>}<button className="icon-button subtle" title="Paylaş" onClick={share}><Share2/></button><button className="icon-button subtle" title={watched?'Takipten çıkar':'Takip et'} onClick={toggleWatch}>{watched?<BookmarkCheck/>:<Bookmark/>}</button></div></header><div className="detail-quote"><small>Güncel fiyat</small><strong>{formatMoney(last)}</strong><span className={positive?'positive':'negative'}>{periodChange>=0?'+':''}{formatMoney(periodChange)} · {periodRate>=0?'+':''}{periodRate.toLocaleString('tr-TR',{maximumFractionDigits:2})}%</span></div><div className={`asset-detail-chart ${positive?'positive':'negative'}`}>{loading?<div className="chart-loading">Grafik hazırlanıyor…</div>:chartPoints.length?<ResponsiveContainer><AreaChart data={chartPoints} margin={{left:8,right:10}}><defs><linearGradient id="assetDetailFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor={positive?'#3d8b68':'#dc5b4f'} stopOpacity=".25"/><stop offset="1" stopColor={positive?'#3d8b68':'#dc5b4f'} stopOpacity="0"/></linearGradient></defs><CartesianGrid vertical={false} stroke="var(--line)" strokeOpacity={0.65}/><XAxis dataKey="timestamp" type="number" scale="time" domain={['dataMin','dataMax']} tickFormatter={(timestamp:number)=>historyLabel(timestamp,range)} axisLine={false} tickLine={false} interval="preserveStartEnd" minTickGap={55} tickMargin={8} height={30} tick={{fill:'var(--muted)',fontSize:10}}/><YAxis orientation="right" width={52} axisLine={false} tickLine={false} tickMargin={6} tickCount={4} domain={tradingChartDomain(chartPoints.map(point=>point.price))} tickFormatter={compactChartTick} tick={{fill:'var(--muted)',fontSize:10}}/><Tooltip content={<ChartValueTooltip formatValue={formatMoney} formatLabel={label=>historyLabel(Number(label),range)}/>}/><Area type="monotone" dataKey="price" stroke={positive?'#3d8b68':'#dc5b4f'} strokeWidth={2.2} fill="url(#assetDetailFill)" dot={false} activeDot={{r:4}}/></AreaChart></ResponsiveContainer>:<div className="chart-loading">Grafik için fiyat verisi yok.</div>}</div>{points.length>1&&<div className="chart-period">{historyPeriod(points)}</div>}<RangeTabs value={range} onChange={setRange}/><section className="position-summary"><h3>Pozisyonum</h3><div><span>Adet<b>{Number(asset.units).toLocaleString('tr-TR')}</b></span><span>Toplam değer<b>{formatMoney(value)}</b></span><span>Ort. fiyat<b>{formatMoney(Number(asset.average_cost))}</b></span><span>Portföy getirisi<b className={gain>=0?'positive':'negative'}>{rate.toLocaleString('tr-TR',{maximumFractionDigits:2})}%</b></span><span>Toplam getiri<b className={gain>=0?'positive':'negative'}>{formatMoney(gain)}</b></span><span>{range} değişim<b className={positive?'positive':'negative'}>{periodRate>=0?'+':''}{periodRate.toLocaleString('tr-TR',{maximumFractionDigits:2})}%</b></span></div></section><section className="asset-history-section"><header><h3>İşlem geçmişi</h3><small>Alım, satım ve ilk kayıt ayrıntıları</small></header><div className="asset-history-list"><div className="asset-history-row buy"><span><b>İlk kayıt</b><small>{initialInfo.date?new Date(`${initialInfo.date}T12:00:00`).toLocaleDateString('tr-TR',{day:'numeric',month:'short',year:'numeric'}):'Tarih yok'}</small></span><span><b>{initialInfo.units.toLocaleString('tr-TR')} adet</b><small>{formatMoney(initialInfo.price)} ort.</small></span><strong>{formatMoney(initialInfo.total)}</strong></div>{transactions.map(item=>{const profit=(Number(item.unit_price)-Number(asset.average_cost))*Number(item.units);return <div className={`asset-history-row ${item.side}`} key={item.id}><span><b>{item.side==='buy'?'Aldım':'Sattım'}</b><small>{new Date(`${item.transaction_date}T12:00:00`).toLocaleDateString('tr-TR',{day:'numeric',month:'short',year:'numeric'})}</small></span><span><b>{Number(item.units).toLocaleString('tr-TR')} adet</b><small>{formatMoney(Number(item.unit_price))} fiyat</small></span><strong className={item.side==='sell'?(profit>=0?'positive':'negative'):undefined}>{item.side==='sell'?`${profit>=0?'+':''}${formatMoney(profit)}`:formatMoney(Number(item.units)*Number(item.unit_price))}</strong></div>})}{!transactions.length&&<p>Henüz alım satım kaydı yok.</p>}</div></section>{fund&&<section className="fund-detail-section"><header><div><h3>Fon bilgileri</h3><p>TEFAS tarafından yayınlanan güncel veriler</p></div><Info/></header><div className="fund-stats"><span>Fon kategorisi<b>{fund.fonKategori||'—'}</b></span><span>Günlük getiri<b className={(fund.gunlukGetiri||0)>=0?'positive':'negative'}>{Number(fund.gunlukGetiri||0).toLocaleString('tr-TR')}%</b></span><span>Fon büyüklüğü<b>{formatMoney(Number(fund.portBuyukluk||0))}</b></span><span>Yatırımcı sayısı<b>{Number(fund.yatirimciSayi||0).toLocaleString('tr-TR')}</b></span><span>Kategori sırası<b>{fund.kategoriDerece&&fund.kategoriFonSay?`${fund.kategoriDerece} / ${fund.kategoriFonSay}`:'—'}</b></span><span>Pazar payı<b>{Number(fund.pazarPayi||0).toLocaleString('tr-TR')}%</b></span></div></section>}{trade&&<div className="detail-trade-actions"><button className="sell" onClick={()=>trade('sell')}>Sattım</button><button className="buy" onClick={()=>trade('buy')}>Aldım</button></div>}</section></div>
}
...
function SettingsPage({theme,setTheme,email,catAlerts,setCatAlerts,openCategories,openGuide,openAccountSwitcher}:{theme:'light'|'dark';setTheme:(x:'light'|'dark')=>void;email:string;catAlerts:boolean;setCatAlerts:(x:boolean)=>void;openCategories:()=>void;openGuide:()=>void;openAccountSwitcher:()=>void}) { const [message,setMessage]=useState(''),[feedbackOpen,setFeedbackOpen]=useState(false),[tradesAffectBalance,setTradesAffectBalance]=useState(()=>localStorage.getItem('asset-trades-affect-balance')==='true');useEffect(()=>localStorage.setItem('asset-trades-affect-balance',String(tradesAffectBalance)),[tradesAffectBalance]); async function resetPassword(){const redirectTo=new URL(import.meta.env.BASE_URL,window.location.origin).toString();const {error}=await supabase.auth.resetPasswordForEmail(email,{redirectTo});if(error){setMessage('İşlem tamamlanamadı.');showAppError('Ayarlardan şifre sıfırlama',error,{e_posta:email})}else setMessage('Şifre sıfırlama bağlantısı e-posta adresinize gönderildi.')} return <><div className="page-content settings-page categorized-settings"><section className="settings-head"><div><h2>Ayarlar</h2><p>{email}</p></div><button className="icon-button" title="Hesap değiştir" onClick={openAccountSwitcher}><ArrowDownUp/></button></section><section className="settings-group"><h3>Hesap ayarları</h3><div className="panel settings-list"><button onClick={openAccountSwitcher}><span className="settings-icon"><ArrowDownUp/></span><span><b>Hesap değiştir</b><small>Kayıtlı hesaplar arasında geçiş yap</small></span><ChevronRight/></button><button onClick={resetPassword}><span className="settings-icon"><KeyRound/></span><span><b>Şifre sıfırla</b><small>{email}</small></span><ChevronRight/></button><button onClick={()=>supabase.auth.signOut()}><span className="settings-icon"><LogOut/></span><span><b>Çıkış yap</b><small>Bu cihazdaki oturumu kapat</small></span><ChevronRight/></button>{message&&<div className="settings-message">{message}</div>}</div></section><section className="settings-group"><h3>Uygulama ayarları</h3><div className="panel settings-list"><div className="settings-row" data-tour="settings"><span className="settings-icon">{theme==='light'?<Sun/>:<Moon/>}</span><span><b>Görünüm</b><small>{theme==='light'?'Açık tema':'Koyu tema'}</small></span><label className="switch"><input type="checkbox" checked={theme==='dark'} onChange={e=>setTheme(e.target.checked?'dark':'light')}/><span/></label></div><div className="settings-row"><span className="settings-icon"><img src={`${import.meta.env.BASE_URL}happy-cat.png`} alt="" /></span><span><b>Kedi bildirimi</b><small>Gelir ve gider kaydında görsel bildirim göster</small></span><label className="switch"><input type="checkbox" checked={catAlerts} onChange={e=>setCatAlerts(e.target.checked)}/><span/></label></div><button onClick={openGuide}><span className="settings-icon"><HelpCircle/></span><span><b>Nasıl kullanılır?</b><small>Ekran üzerinde adım adım göster</small></span><ChevronRight/></button><button onClick={openCategories}><span className="settings-icon"><Tags/></span><span><b>Kategoriler</b><small>Gelir ve gider kategorilerini yönetin</small></span><ChevronRight/></button></div></section><section className="settings-group"><h3>Varlık ayarları</h3><div className="panel settings-list"><div className="settings-row"><span className="settings-icon"><ArrowDownUp/></span><span><b>Varlık alım satımını aylık dengeye yansıt</b><small>Alımlar gider, satışlar gelir olarak özete işlensin</small></span><label className="switch"><input type="checkbox" checked={tradesAffectBalance} onChange={e=>setTradesAffectBalance(e.target.checked)}/><span/></label></div></div></section><section className="settings-group"><h3>Destek</h3><div className="panel settings-list"><button onClick={()=>setFeedbackOpen(true)}><span className="settings-icon"><MessageCircle/></span><span><b>Öneri ve geri bildirim</b><small>Görüşlerinizi doğrudan bize iletin</small></span><ChevronRight/></button></div></section></div>{feedbackOpen&&<FeedbackModal reporterEmail={email} close={()=>setFeedbackOpen(false)}/>}</> }
...
function AssetModal({userId,assets,preset,close,saved}:{userId:string;assets:Asset[];preset:Asset|null;close:()=>void;saved:()=>void}) {
  ...
  return <Modal title="Varlık ekle" close={clearAndClose} className={step===symbolStep?'asset-symbol-modal':''}><form className="asset-wizard" onSubmit={submit}>
    ...
    {step===symbolStep&&<section className="wizard-pane"><label className="search asset-search"><Search/><input placeholder={`${kind==='Kıymetli Madenler'?'Madenler':kind} ara`} value={query} onChange={e=>setQuery(e.target.value)}/></label><div className="symbol-results">{matches.slice(0,150).map(item=><button type="button" key={`${item.kind}-${item.exchange||'global'}-${item.dataSource||'yahoo'}-${item.symbol}`} title={`${item.symbol} - ${item.name}`} aria-label={`${item.symbol} - ${item.name}`} className={selected===item?'active':''} onClick={()=>setSelected(item)}><b>{item.symbol}</b><span>{item.name}</span></button>)}</div>{matches.length>150&&<small className="result-limit">{matches.length.toLocaleString('tr-TR')} sonuç içinde arama yapabilirsin.</small>}</section>}
...
function ConfirmModal({title,message,confirmLabel,close,onConfirm}:{title:string;message:string;confirmLabel:string;close:()=>void;onConfirm:()=>void|Promise<void>}) {
  const [busy,setBusy]=useState(false)
  async function confirmAction(){setBusy(true);await onConfirm();setBusy(false);close()}
  return <Modal title={title} close={close} className="confirm-modal"><div className="confirm-content"><p>{message}</p><div className="form-actions"><button type="button" onClick={close} disabled={busy}>Vazgeç</button><button type="button" className="primary danger-primary" disabled={busy} onClick={confirmAction}>{busy?'Siliniyor…':confirmLabel}</button></div></div></Modal>
}
