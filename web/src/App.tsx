...
function ErrorReportModal({report,reporterEmail,close}:{report:AppErrorReport;reporterEmail:string;close:()=>void}) {
  ...
  async function sendReport(){setBusy(true);setSendFailed(false);const {data,error}=await supabase.functions.invoke('report-app-error',{body:{...report,reporterEmail,page:document.querySelector('.workspace h1')?.textContent||document.title,url:window.location.href,occurredAt:new Date().toISOString(),userAgent:navigator.userAgent}});setBusy(false);if(error||!data?.sent){setSendFailed(true);return}setSent(true)}
  return <Modal title="Sistemde bir hata oluştu" close={close}><div className="error-report-content"><span className="error-report-icon"><TriangleAlert/></span>{sent?<><h3>Bildirim gönderildi</h3><p>Hata ayrıntıları incelememiz için iletildi.</p></>:<><h3>İşlem tamamlanamadı</h3><p>İstersen bu hatayı, yaptığın işlemin bilgileriyle birlikte bize bildirebilirsin.</p><div className="error-report-action"><small>İşlem</small><b>{report.action}</b></div>{sendFailed&&<p className="error-report-failed">Bildirim gönderilemedi. Biraz sonra tekrar deneyebilirsin.</p>}</>}<div className="form-actions">{!sent&&<button type="button" onClick={close}>Vazgeç</button>}{sent?<button type="button" className="primary" onClick={close}>Kapat</button>:<button type="button" className="primary" disabled={busy} onClick={sendReport}><Send/>{busy?'Gönderiliyor…':'Bildir'}</button>}</div></div></Modal>
}
  ...
  return <div className="app-shell sidebar-auto">
    ...
    <main className="workspace"><header><div><span className="mobile-logo">Harcamaç</span><h1>{title}</h1><p>{subtitle}</p></div><div className="header-actions">{(page === 'transactions' || page === 'dashboard') && <button className="category-button" data-tour="category-action" title="Kategoriler" onClick={() => setCategoryOpen(true)}><Tags /><span>Kategoriler</span></button>}<button className="primary" data-tour="top-action" onClick={topAction.onClick}><Plus /> <span>{topAction.label}</span></button></div></header>
      {loading ? <div className="loading">Verileriniz getiriliyor…</div> : <>
        ...
        {page === 'assets' && <Assets assets={assets} entries={entries} onDelete={async id => { const asset=assets.find(item=>item.id===id);const {error}=await supabase.from('assets').delete().eq('id', id);if(error)showAppError('Varlık silme',error,{sembol:asset?.symbol||'',varlık:asset?.name||'',varlık_id:id});else loadData() }} onChanged={loadData} />}
...
function FeedbackModal({reporterEmail,close}:{reporterEmail:string;close:()=>void}) {
  ...
  async function submit(event:FormEvent){event.preventDefault();const message=feedback.trim();if(!message)return;setBusy(true);setSendFailed(false);const {data,error}=await supabase.functions.invoke('report-app-error',{body:{type:'feedback',message,reporterEmail,page:'Ayarlar',url:window.location.href,occurredAt:new Date().toISOString(),userAgent:navigator.userAgent}});setBusy(false);if(error||!data?.sent){setSendFailed(true);return}setSent(true)}
