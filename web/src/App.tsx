...
function Assets({userId,assets,entries,onBuy,onDelete,onChanged}:{userId:string;assets:Asset[];entries:Entry[];onBuy:(asset:Asset)=>void;onDelete:(asset:Asset)=>void;onChanged:()=>void}) {
  ...
  const [performanceData,setPerformanceData]=useState<{label:string;value:number|null;changeRate?:number}[]>([])
  ...
  const latestHistoricalValue = [...performanceData].reverse().find(point => Number(point.value) > 0)?.value
  const analysisCurrentValue = analysisMode === 'performance' && latestHistoricalValue ? latestHistoricalValue : total
  ...
  useEffect(()=>{let cancelled=false;setPerformanceLoading(true);const end=Date.now(),start=portfolioRangeStart(range,positions).getTime();Promise.all(positions.map(async asset=>({asset,history:await fetchAssetHistory(asset,range)}))).then(series=>{if(cancelled)return;const purchaseTimes=positions.flatMap(asset=>{if(!asset.purchase_date)return [];const timestamp=new Date(`${asset.purchase_date}T12:00:00`).getTime();return Number.isFinite(timestamp)&&timestamp>=start&&timestamp<=end?[timestamp]:[]});const timestamps=[...new Set([start,...purchaseTimes,...series.flatMap(item=>item.history.map(point=>point.timestamp*1000)).filter(timestamp=>timestamp>=start&&timestamp<=end),end])].sort((a,b)=>a-b);const raw=timestamps.map(timestamp=>{let value=0;for(const {asset,history} of series){const purchase=asset.purchase_date?new Date(`${asset.purchase_date}T12:00:00`).getTime():start;if(timestamp<purchase)continue;const point=[...history].reverse().find(item=>item.timestamp*1000<=timestamp);const assetCost=Number(asset.units)*Number(asset.average_cost);const price=(point?.price??Number(asset.average_cost))||Number(asset.current_price);value+=analysisMode==='flow'?assetCost:price*Number(asset.units)}return {label:historyLabel(timestamp/1000,range),value:value>0?value:null}});let data=raw.filter(point=>Number(point.value)>0);if(data.length===1){const only=data[0];data=zeroRangePoints(range).map(point=>({label:point.label,value:only.value}))}const baseline=data.find(point=>Number(point.value)>0)?.value||0;setPerformanceData(data.map(point=>({...point,changeRate:point.value&&baseline?pointRate(point.value,baseline):0})))}).finally(()=>{if(!cancelled)setPerformanceLoading(false)});return()=>{cancelled=true}},[portfolioHistoryKey,range,analysisMode])
  useEffect(()=>{if(analysisMode!=='performance')return;setPerformanceData(data=>{const baseline=data.find(point=>Number(point.value)>0)?.value||0;return data.length?data.map((point,index)=>index===data.length-1?{...point,value:total>0?total:null,changeRate:baseline?pointRate(total,baseline):0}:point):data})},[analysisMode,total])
