...
function EntryModal({userId,categories,entry,close,saved,onCategoryAdded}:{userId:string;categories:Category[];entry:Entry|null;close:()=>void;saved:(kind:FlowKind)=>void;onCategoryAdded:(category:Category)=>void}) {
  ...
  const available=useMemo(()=>categories.filter(c=>c.kind===kind&&(cadence==='recurring'?c.recurring:c.one_time)),[categories,kind,cadence])
  useEffect(()=>{if(!available.some(c=>c.id===category))setCategory(available[0]?.id||'')},[kind,cadence,categories,category,available])
  useEffect(()=>{if(!quickOpen){setQuickOne(cadence==='one_time');setQuickRecurring(cadence==='recurring')}},[cadence,quickOpen])
  async function addQuickCategory(){if(!quickName.trim()||(!quickOne&&!quickRecurring))return;setBusy(true);const {data,error}=await supabase.from('categories').insert({user_id:userId,name:quickName.trim(),color:quickColor,kind,one_time:quickOne,recurring:quickRecurring}).select('*').single();setBusy(false);if(error)return alert(error.message);if(data){const created=data as Category;onCategoryAdded(created);setCategory(created.id);setQuickName('');setQuickOpen(false)}}
...
function GuideModal({goTo,close}:{goTo:(page:Page)=>void;close:()=>void}) {
  ...
  useEffect(()=>{
    function measure(){const nodes=Array.from(document.querySelectorAll(`[data-tour="${item.spot}"]`)) as HTMLElement[];const node=nodes.find(el=>{const r=el.getBoundingClientRect();return r.width>0&&r.height>0});if(!node){setRect(null);return}const r=node.getBoundingClientRect(), pad=10;setRect({left:Math.max(8,r.left-pad),top:Math.max(8,r.top-pad),width:Math.min(window.innerWidth-16,r.width+pad*2),height:Math.min(window.innerHeight-16,r.height+pad*2)})}
    const raf=window.requestAnimationFrame(()=>window.setTimeout(measure,60));window.addEventListener('resize',measure);return()=>{window.cancelAnimationFrame(raf);window.removeEventListener('resize',measure)}
  },[item.spot])
