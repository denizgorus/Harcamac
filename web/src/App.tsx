...
function Dashboard({ entries, assets }: { entries: Entry[]; assets: Asset[] }) {
  ...
  function shiftMonth(step:number){const next=new Date(selectedDate);next.setMonth(next.getMonth()+step);setSelectedMonth(monthKey(next))}
  return <div className="page-content">
    <section className="dashboard-period"><div><small>Özet dönemi</small><b>{monthLabel}</b></div><div><button className="icon-button subtle" title="Önceki ay" onClick={()=>shiftMonth(-1)}><ChevronLeft/></button><input type="month" value={selectedMonth} onChange={event=>setSelectedMonth(event.target.value||monthKey(new Date()))}/><button className="icon-button subtle" title="Sonraki ay" onClick={()=>shiftMonth(1)}><ChevronRight/></button></div></section>
...
function Assets({userId,assets,entries,onBuy,onDelete,onChanged}:{userId:string;assets:Asset[];entries:Entry[];onBuy:(asset:Asset)=>void;onDelete:(asset:Asset)=>void;onChanged:()=>void}) {
  ...
  const activeChartData = performanceData.map(point=>({...point,chartValue:analysisMode==='performance'?(performanceUnit==='percent'?point.performanceRate:point.performance):point.flow,changeRate:analysisMode==='flow'?point.flowRate:point.performanceRate}))
  ...
  return <div className="page-content assets-page midas-assets">
    <section className="assets-portfolio">
      ...
      <div className={`portfolio-change ${rangeGain >= 0 ? 'positive' : 'negative'}`}><span>{hidden?'******':`${rangeGain>=0?'+':''}${formatMoney(rangeGain)}`}</span><b>{rangeGainRate.toLocaleString('tr-TR',{maximumFractionDigits:2})}%</b></div>
      <RangeTabs value={range} onChange={setRange} className="portfolio-range-tabs"/>
