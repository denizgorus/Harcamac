...
import { ArrowDownUp, BarChart3, CalendarDays, ChevronRight, CircleDollarSign, HelpCircle, LayoutDashboard, LogOut, Menu, Moon, Pencil, PieChart as PieIcon, Plus, Search, Settings, Sun, Tags, Trash2, WalletCards, X } from 'lucide-react'
...
const guidePages = [
  { title: 'Özet', text: 'Aylık gelir, gider, denge ve varlık toplamınızı tek ekranda görürsünüz. Grafikler, ay içindeki para akışını ve gider dağılımını hızlıca okumanızı sağlar.' },
  { title: 'Hareketler', text: 'Tek seferlik veya düzenli gelir-gider kayıtları buradan yönetilir. Kayıtları tarihe, tutara, türe ve kategoriye göre süzebilirsiniz.' },
  { title: 'Düzenli', text: 'Kira, fatura, abonelik ve maaş gibi tekrarlayan hareketler burada toplanır. Aylık planınızı gelir ve gider olarak ayrı takip edebilirsiniz.' },
  { title: 'Varlıklar', text: 'Nakit, banka, altın, hisse veya kripto gibi birikimlerinizi ekleyip toplam portföyünüzü izleyebilirsiniz.' },
  { title: 'Ayarlar', text: 'Tema seçimi, kategori yönetimi, kullanım rehberi ve oturum işlemleri burada bulunur.' }
]
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><header><div><span className="mobile-logo">Harcamac</span><h1>{title}</h1><p>{page === 'dashboard' ? 'Finansal durumunuza genel bakış' : page === 'transactions' ? 'Tüm gelir ve gider kayıtlarınız' : ''}</p></div><div className="header-actions">{page === 'transactions' && <button className="icon-button" title="Kategoriler" onClick={() => setCategoryOpen(true)}><Tags /></button>}<button className="primary" onClick={() => { setEditEntry(null); setEntryOpen(true) }}><Plus /> <span>Yeni hareket</span></button></div></header>
      {loading ? <div className="loading">Verileriniz getiriliyor…</div> : <>
        ...
        {page === 'settings' && <SettingsPage theme={theme} setTheme={setTheme} email={session.user.email || ''} openCategories={() => setCategoryOpen(true)} openGuide={() => setGuideOpen(true)} />}
      </>}
    </main>
...
function SettingsPage({theme,setTheme,email,openCategories,openGuide}:{theme:'light'|'dark';setTheme:(x:'light'|'dark')=>void;email:string;openCategories:()=>void;openGuide:()=>void}) { return <div className="page-content settings-page"><section className="panel settings-list"><div className="settings-row"><span className="settings-icon">{theme==='light'?<Sun/>:<Moon/>}</span><span><b>Görünüm</b><small>{theme==='light'?'Açık tema':'Koyu tema'}</small></span><label className="switch"><input type="checkbox" checked={theme==='dark'} onChange={e=>setTheme(e.target.checked?'dark':'light')}/><span/></label></div><button onClick={openGuide}><span className="settings-icon"><HelpCircle/></span><span><b>Nasıl kullanılır?</b><small>Başlangıç bilgilendirmesini tekrar açın</small></span><ChevronRight/></button><button onClick={openCategories}><span className="settings-icon"><Tags/></span><span><b>Kategoriler</b><small>Gelir ve gider kategorilerini yönetin</small></span><ChevronRight/></button><button onClick={()=>supabase.auth.signOut()}><span className="settings-icon"><LogOut/></span><span><b>Çıkış yap</b><small>{email}</small></span><ChevronRight/></button></section></div> }
...
function GuideModal({close}:{close:()=>void}) { return <Modal title="Harcamac nasıl kullanılır?" close={close}><div className="guide-content"><p className="guide-lead">Harcamac, gelirlerinizi, giderlerinizi ve varlıklarınızı günlük kullanımda hızlıca takip etmeniz için tasarlandı.</p><div className="guide-grid">{guidePages.map((page,i)=><article key={page.title}><small>{String(i+1).padStart(2,'0')}</small><b>{page.title}</b><p>{page.text}</p></article>)}</div><div className="form-actions"><button className="primary" onClick={close}>Tamam</button></div></div></Modal> }
