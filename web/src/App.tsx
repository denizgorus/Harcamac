...
import { AssetCatalogItem, FundSnapshot, MarketRange, assetCatalog, fetchAssetHistory, fetchBistPrices, fetchFundSnapshot, fetchHistoricalAssetPrice } from './lib/marketPrices'
...
function Assets({assets,entries,onDelete,onChanged}:{assets:Asset[];entries:Entry[];onDelete:(id:string)=>void;onChanged:()=>void}) {
  ...
  useEffect(()=>{let cancelled=false;setPerformanceLoading(true);const end=Date.now(),start=end-assetRangeDays[range]*86400000;Promise.all(assets.map(async asset=>({asset,history:await fetchAssetHistory(asset,range)}))).then(series=>{if(cancelled)return;const timestamps=[...new Set(series.flatMap(item=>item.history.map(point=>point.timestamp*1000)).filter(timestamp=>timestamp>=start))].sort((a,b)=>a-b);const sampled=timestamps.filter((_,index)=>timestamps.length<=70||index%Math.ceil(timestamps.length/70)===0);const data=sampled.map(timestamp=>{let value=0;for(const {asset,history} of series){const purchase=asset.purchase_date?new Date(`${asset.purchase_date}T12:00:00`).getTime():start;if(timestamp<purchase)continue;const point=[...history].reverse().find(item=>item.timestamp*1000<=timestamp);const assetCost=Number(asset.units)*Number(asset.average_cost);value+=analysisMode==='flow'?assetCost:(point?.price??Number(asset.current_price))*Number(asset.units)}return {label:new Date(timestamp).toLocaleDateString('tr-TR',{day:'numeric',month:'short'}),value}});setPerformanceData(data.length>1?data:[{label:'Başlangıç',value:cost},{label:'Bugün',value:analysisMode==='flow'?cost:total}])}).finally(()=>{if(!cancelled)setPerformanceLoading(false)});return()=>{cancelled=true}},[assets,range,analysisMode,monthlyBalance])
...
function MarketDiscover({formatMoney,watched,toggleWatch,openItem}:{formatMoney:(value:number)=>string;watched:string[];toggleWatch:(item:AssetCatalogItem)=>void;openItem:(item:AssetCatalogItem)=>void}) {
  ...
  useEffect(()=>{let cancelled=false;Promise.allSettled(featured.map(async item=>{const asset={id:`discover-${item.symbol}`,user_id:'',name:item.name,symbol:item.symbol,kind:item.kind,units:1,average_cost:0,current_price:0};const history=await fetchAssetHistory(asset,'1G'),price=(await fetchHistoricalAssetPrice(item,today))||0,first=history[0]?.price||0,last=history[history.length-1]?.price||0,rawChange=first&&last?((last-first)/first)*100:0;return {key:`${item.kind}:${item.symbol}`,price,change:Math.abs(rawChange)<=100?rawChange:0}})).then(values=>{if(!cancelled)setQuotes(Object.fromEntries(values.flatMap(value=>value.status==='fulfilled'?[[value.value.key,value.value]]:[])))});return()=>{cancelled=true}},[featured])
...
function AssetDetail({asset,monthlyBalance,formatMoney,watched,close,toggleWatch,trade}:{asset:Asset;monthlyBalance:number;formatMoney:(value:number)=>string;watched:boolean;close:()=>void;toggleWatch:()=>void;trade?:(side:'buy'|'sell')=>void}) {
  const [range,setRange]=useState<MarketRange>('1A'),[points,setPoints]=useState<{label:string;price:number;value:number}[]>([]),[loading,setLoading]=useState(true),[fund,setFund]=useState<FundSnapshot|null>(null)
