...
function Assets({userId,assets,entries,onBuy,onDelete,onChanged}:{userId:string;assets:Asset[];entries:Entry[];onBuy:(asset:Asset)=>void;onDelete:(asset:Asset)=>void;onChanged:()=>void}) {
  ...
  useEffect(()=>{let cancelled=false;setPerformanceLoading(true);const end=Date.now(),start=portfolioRangeStart(range,positions).getTime();Promise.all(positions.map(async asset=>({asset,history:await fetchAssetHistory(asset,range)}))).then(series=>{if(cancelled)return;const timestamps=[...new Set([start,...series.flatMap(item=>item.history.map(point=>point.timestamp*1000)).filter(timestamp=>timestamp>=start&&timestamp<=end),end])].sort((a,b)=>a-b);const data=timestamps.map(timestamp=>{let value=showMonthlyBalance?monthlyBalance:0;for(const {asset,history} of series){const purchase=asset.purchase_date?new Date(`${asset.purchase_date}T12:00:00`).getTime():start;if(timestamp<purchase)continue;const point=[...history].reverse().find(item=>item.timestamp*1000<=timestamp);const assetCost=Number(asset.units)*Number(asset.average_cost);const price=(point?.price??Number(asset.average_cost))||Number(asset.current_price);value+=analysisMode==='flow'?assetCost:price*Number(asset.units)}return {label:historyLabel(timestamp/1000,range),value}});setPerformanceData(data.length>1?data:zeroRangePoints(range).map(point=>({label:point.label,value:showMonthlyBalance?monthlyBalance:0})))}).finally(()=>{if(!cancelled)setPerformanceLoading(false)});return()=>{cancelled=true}},[portfolioHistoryKey,range,analysisMode,monthlyBalance,showMonthlyBalance])
  useEffect(()=>{if(analysisMode!=='performance')return;setPerformanceData(data=>data.length?data.map((point,index)=>index===data.length-1?{...point,value:total}:point):data)},[analysisMode,total])
