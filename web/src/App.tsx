...
function Dashboard({ entries, assets }: { entries: Entry[]; assets: Asset[] }) {
  ...
  const assetTotal = mergeAssetPositions(assets).reduce((sum, asset) => sum + resolvedAssetValue(asset, monthlyBalance), 0)+(localStorage.getItem('asset-monthly-balance-visible')==='true'?monthlyBalance:0)
...
function Assets({userId,assets,entries,onBuy,onDelete,onChanged}:{userId:string;assets:Asset[];entries:Entry[];onBuy:(asset:Asset)=>void;onDelete:(asset:Asset)=>void;onChanged:()=>void}) {
  ...
  const [showMonthlyBalance,setShowMonthlyBalance]=useState(()=>localStorage.getItem('asset-monthly-balance-visible')==='true')
  const assetTotal = positions.reduce((sum, asset) => sum + resolvedAssetValue(asset, monthlyBalance), 0)
  const total = assetTotal+(showMonthlyBalance?monthlyBalance:0)
  ...
  const groupedAssets=Object.values(positions.reduce((acc,asset)=>{const name=normalizedAssetKind(asset.kind)||'Diğer';acc[name]||={name,items:[],value:0,cost:0};acc[name].items.push(asset);acc[name].value+=resolvedAssetValue(asset,monthlyBalance);acc[name].cost+=Number(asset.units)*Number(asset.average_cost);return acc},{} as Record<string,{name:string;items:Asset[];value:number;cost:number}>)).sort((a,b)=>b.value-a.value)
  ...
  useEffect(()=>{localStorage.setItem('asset-monthly-balance-visible',String(showMonthlyBalance))},[showMonthlyBalance])
  ...
  useEffect(()=>{let cancelled=false;setPerformanceLoading(true);const end=Date.now(),start=marketRangeStart(range).getTime();Promise.all(positions.map(async asset=>({asset,history:await fetchAssetHistory(asset,range)}))).then(series=>{if(cancelled)return;const timestamps=[...new Set(series.flatMap(item=>item.history.map(point=>point.timestamp*1000)).filter(timestamp=>timestamp>=start&&timestamp<=end))].sort((a,b)=>a-b);const data=timestamps.map(timestamp=>{let value=showMonthlyBalance?monthlyBalance:0;for(const {asset,history} of series){const purchase=asset.purchase_date?new Date(`${asset.purchase_date}T12:00:00`).getTime():start;if(timestamp<purchase)continue;const point=[...history].reverse().find(item=>item.timestamp*1000<=timestamp);const assetCost=Number(asset.units)*Number(asset.average_cost);value+=analysisMode==='flow'?assetCost:(point?.price??Number(asset.current_price))*Number(asset.units)}return {label:historyLabel(timestamp/1000,range),value}});setPerformanceData(data.length>1?data:zeroRangePoints(range).map(point=>({label:point.label,value:showMonthlyBalance?monthlyBalance:0})))}).finally(()=>{if(!cancelled)setPerformanceLoading(false)});return()=>{cancelled=true}},[positions,range,analysisMode,monthlyBalance,showMonthlyBalance])
  ...
  return <div className="page-content assets-page midas-assets">
    <section className="assets-portfolio">
      ...
      <label className="portfolio-balance-switch"><span><b>Aylık dengeyi dahil et</b><small>{formatMoney(monthlyBalance)}</small></span><span className="switch"><input type="checkbox" checked={showMonthlyBalance} onChange={event=>setShowMonthlyBalance(event.target.checked)}/><span/></span></label>
      <div className="portfolio-meta"><span>Maliyet <b>{formatMoney(cost)}</b></span><span>{currency==='USD'&&usdTry>1?`1 USD = ${trMoney.format(usdTry)}`:`${positions.length.toLocaleString('tr-TR')} varlık`}</span></div>
...
function AssetRow({asset,total,monthlyBalance,formatMoney,onOpen,onDelete}:{asset:Asset;total:number;monthlyBalance:number;formatMoney:(value:number)=>string;onOpen:()=>void;onDelete:(asset:Asset)=>void}) {
  ...
  return <div className="asset-row clickable" onClick={onOpen}>
    ...
    <button className="icon-button subtle danger" title="Varlığı sil" onClick={event=>{event.stopPropagation();if(confirm('Bu varlık silinsin mi?'))onDelete(asset)}}><Trash2/></button>
  </div>
}
