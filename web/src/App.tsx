...
function ChartBody({widget,entries}:{widget:ChartWidget;entries:Entry[]}) { const data=chartData(widget,entries), colors=['#dc5b4f','#e7a23d','#418a73','#477ea8','#8b69a8','#6b7c55']; const total=data.reduce((s:any,x:any)=>s+Number(x.value||0),0); if(widget.chart==='pie'||widget.metric==='categoryExpense')return <div className="donut-wrap"><div className="donut"><ResponsiveContainer><PieChart><Pie data={data.length?data:[{name:'Veri yok',value:1}]} innerRadius="58%" outerRadius="82%" dataKey="value" stroke="none">{(data.length?data:[{}]).map((x:any,i)=><Cell key={i} fill={data.length?(x.color||colors[i%colors.length]):'#e4e6e1'}/>)}</Pie><Tooltip allowEscapeViewBox={{x:true,y:true}} offset={18} formatter={(v:number)=>trMoney.format(v)}/></PieChart></ResponsiveContainer><div><b>{trMoney.format(total)}</b><small>toplam</small></div></div><div className="legend">{data.map((x:any,i)=><div key={x.name}><span style={{background:x.color||colors[i%colors.length]}}/><span className="legend-name">{x.name}</span><span className="legend-amount">{trMoney.format(x.value)}</span><b>{total?Math.round(x.value/total*100):0}%</b></div>)}</div></div>; return <div className="chart"><ResponsiveContainer>{widget.chart==='area'?<AreaChart data={data}><CartesianGrid vertical={false} stroke="#dfe2dc"/><XAxis dataKey="name" axisLine={false} tickLine={false}/><YAxis hide/><Tooltip formatter={(v:number)=>trMoney.format(v)}/>{widget.metric!=='expense'&&<Area dataKey="Gelir" fill="#3d8b68" stroke="#3d8b68" fillOpacity={.18}/>} {widget.metric!=='income'&&<Area dataKey="Gider" fill="#dc5b4f" stroke="#dc5b4f" fillOpacity={.16}/>}</AreaChart>:<BarChart data={data}><CartesianGrid vertical={false} stroke="#dfe2dc"/><XAxis dataKey="name" axisLine={false} tickLine={false}/><YAxis hide/><Tooltip formatter={(v:number)=>trMoney.format(v)}/>{widget.metric!=='expense'&&<Bar dataKey="Gelir" fill="#3d8b68" radius={[4,4,0,0]}/>} {widget.metric!=='income'&&<Bar dataKey="Gider" fill="#dc5b4f" radius={[4,4,0,0]}/>}</BarChart>}</ResponsiveContainer></div> }
